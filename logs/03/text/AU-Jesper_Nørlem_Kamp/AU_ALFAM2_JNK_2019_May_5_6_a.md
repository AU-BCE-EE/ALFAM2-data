---
title: 'Submitted data summary'
---

# Submitter

```
## [1] "Jesper Nørlem Kamp"
```

# File

```
## [1] "../../data-submitted/03/AU/ALFAM2_JNK_2019_May_5_6_a.xlsx"
```

# Graphical summary

```
## e.cum.final   e.cum.final   e.cum.final   e.cum.final   e.cum.final   
## 
## e.cum.final  vs. pmid 
##     +--+------------+------------+-------------+------------+--+
##     |                            *                             |
##     |                                                          |
## 6.2 +                                                          +
##     |                                                          |
##     |                                                          |
##     |                                                          |
##   6 +                                                          +
##     |                                                          |
##     |                                                       *  |
##     |                                                          |
## 5.8 +                                                          +
##     |                                                          |
##     |  *                                                       |
##     +--+------------+------------+-------------+------------+--+
##        1           1.5           2            2.5           3   
## 
## e.cum.final  vs. row.in.file.plot 
## 7 +-----------+----------------+----------------+--------------+
##   |                                                            |
##   |                                                            |
##   |                                                            |
## 6 +                                                            +
##   |                                                            |
##   |                                                            |
## 5 +  *                *                                    *   +
##   |                                                            |
##   |                                                            |
## 4 +                                                            +
##   |                                                            |
##   |                                                            |
##   |                                                            |
## 3 +-----------+----------------+----------------+--------------+
##              5.8               6               6.2              
## 
## stem and leaf plot of  e.cum.final 
## 
##   The decimal point is 1 digit(s) to the left of the |
## 
##   56 | 0
##   58 | 0
##   60 | 
##   62 | 3
## 
## e.rel.final   e.rel.final   e.rel.final   e.rel.final   e.rel.final   
## 
## e.rel.final  vs. pmid 
##       +--+------------+-----------+------------+------------+--+
##       |                           *                            |
## 0.116 +                                                        +
##       |                                                        |
## 0.114 +                                                        +
##       |                                                        |
## 0.112 +                                                        +
##       |                                                        |
##       |                                                        |
##  0.11 +                                                     *  +
##       |                                                        |
## 0.108 +                                                        +
##       |                                                        |
## 0.106 +  *                                                     +
##       +--+------------+-----------+------------+------------+--+
##          1           1.5          2           2.5           3   
## 
## e.rel.final  vs. row.in.file.plot 
## 7 +----+--------+--------+--------+--------+--------+----------+
##   |                                                            |
##   |                                                            |
##   |                                                            |
## 6 +                                                            +
##   |                                                            |
##   |                                                            |
## 5 +  *                *                                   *    +
##   |                                                            |
##   |                                                            |
## 4 +                                                            +
##   |                                                            |
##   |                                                            |
##   |                                                            |
## 3 +----+--------+--------+--------+--------+--------+----------+
##      0.106    0.108    0.11     0.112    0.114    0.116         
## 
## stem and leaf plot of  e.rel.final 
## 
##   The decimal point is 2 digit(s) to the left of the |
## 
##   10 | 69
##   11 | 
##   11 | 7
## 
## app.rate   app.rate   app.rate   app.rate   app.rate   
## 
## app.rate  vs. pmid 
##    +--+------------+-------------+-------------+------------+--+
## 40 +                                                           +
##    |                                                           |
##    |                                                           |
## 35 +                                                           +
##    |                                                           |
##    |                                                           |
## 30 +  *                          *                          *  +
##    |                                                           |
##    |                                                           |
## 25 +                                                           +
##    |                                                           |
##    |                                                           |
## 20 +                                                           +
##    +--+------------+-------------+-------------+------------+--+
##       1           1.5            2            2.5           3   
## 
## app.rate  vs. row.in.file.plot 
## 7 +----+------------+-----------+------------+------------+----+
##   |                                                            |
##   |                                                            |
##   |                                                            |
## 6 +                                                            +
##   |                                                            |
##   |                                                            |
## 5 +                             *                              +
##   |                                                            |
##   |                                                            |
## 4 +                                                            +
##   |                                                            |
##   |                                                            |
##   |                                                            |
## 3 +----+------------+-----------+------------+------------+----+
##       20           25          30           35           40     
## 
## stem and leaf plot of  app.rate 
## 
##   The decimal point is 1 digit(s) to the right of the |
## 
##   3 | 000
## 
## man.dm   man.dm   man.dm   man.dm   man.dm   
## 
## man.dm  vs. pmid
```

```
## Error in `txtplot()`:
## ! x and y need to be of type numeric
```

# Numeric summary

```
## 
##  3 rows and 225 columns
##  3 unique rows
##                                                                                                                                                                                      cpmid
## Class                                                                                                                                                                            character
## Minimum                D:1.I:AU.Pr:Vejrumbro_2019.F:../../data-submitted/03/AU/ALFAM2_JNK_2019_May_5_6_a.xlsx.E:Vejrumbro_2019_May.F:.P:1.T:AGM.R:1.R2:.T:2019-05-21 06:00:00.M:bLSCRDS_1m
## Maximum            D:1.I:AU.Pr:Vejrumbro_2019.F:../../data-submitted/03/AU/ALFAM2_JNK_2019_May_5_6_a.xlsx.E:Vejrumbro_2019_May.F:.P:1.T:CRDS_2m.R:1.R2:.T:2019-05-21 06:00:00.M:bLSCRDS_1m
## Mean                                                                                                                                                                                  <NA>
## Unique (excld. NA)                                                                                                                                                                       3
## Missing values                                                                                                                                                                           0
## Sorted                                                                                                                                                                                TRUE
##                                                                                                                                                                                           
##                       pub.id           proj              exper     field
## Class              character      character          character character
## Minimum               Atmos1 Vejrumbro_2019 Vejrumbro_2019_May          
## Maximum               Atmos1 Vejrumbro_2019 Vejrumbro_2019_May          
## Mean                    <NA>           <NA>               <NA>      <NA>
## Unique (excld. NA)         1              1                  1         1
## Missing values             0              0                  0         0
## Sorted                  TRUE           TRUE               TRUE      TRUE
##                                                                         
##                         plot       rep plot.area     lat    long   country
## Class              character character   numeric numeric numeric character
## Minimum                    1         1    260000    56.5    9.54        DK
## Maximum                    1         1    260000    56.5    9.54        DK
## Mean                    <NA>      <NA>    260000    56.5    9.54      <NA>
## Unique (excld. NA)         1         1         1       1       1         1
## Missing values             0         0         0       0       0         0
## Sorted                  TRUE      TRUE      TRUE    TRUE    TRUE      TRUE
##                                                                           
##                         topo    clay    silt    sand      oc soil.type
## Class              character numeric numeric numeric numeric character
## Minimum                 Flat    <NA>    <NA>    <NA>    <NA>      Loam
## Maximum                 Flat    <NA>    <NA>    <NA>    <NA>      Loam
## Mean                    <NA>    <NA>    <NA>    <NA>    <NA>      <NA>
## Unique (excld. NA)         1       0       0       0       0         1
## Missing values             0       3       3       3       3         0
## Sorted                  TRUE    <NA>    <NA>    <NA>    <NA>      TRUE
##                                                                       
##                    soil.water soil.water.v soil.moist   soil.ph soil.dens
## Class               character      numeric  character character character
## Minimum                  <NA>         <NA>       <NA>      <NA>      <NA>
## Maximum                  <NA>         <NA>       <NA>      <NA>      <NA>
## Mean                     <NA>         <NA>       <NA>      <NA>      <NA>
## Unique (excld. NA)          0            0          0         0         0
## Missing values              3            3          3         3         3
## Sorted                   <NA>         <NA>       <NA>      <NA>      <NA>
##                                                                          
##                     crop.res      till man.source man.source.det   man.bed
## Class              character character     factor      character character
## Minimum                   No        No        cat           <NA>      <NA>
## Maximum                   No        No       none           <NA>      <NA>
## Mean                    <NA>      <NA>        mix           <NA>      <NA>
## Unique (excld. NA)         1         1          1              0         0
## Missing values             0         0          0              3         3
## Sorted                  TRUE      TRUE       TRUE           <NA>      <NA>
##                                                                           
##                    man.con            man.trt1  man.trt2 man.stor    man.dm
## Class               factor           character character  numeric character
## Minimum             slurry Anaerobic digestion      <NA>     <NA>      5.35
## Maximum              solid Anaerobic digestion      <NA>     <NA>      5.35
## Mean                slurry                <NA>      <NA>     <NA>      <NA>
## Unique (excld. NA)       1                   1         0        0         1
## Missing values           0                   0         3        3         0
## Sorted                TRUE                TRUE      <NA>     <NA>      TRUE
##                                                                            
##                       man.vs   man.tkn man.tan man.tic  man.ua man.vfa
## Class              character character numeric logical logical numeric
## Minimum                 <NA>       3.1     1.8    <NA>    <NA>    <NA>
## Maximum                 <NA>       3.1     1.8    <NA>    <NA>    <NA>
## Mean                    <NA>      <NA>     1.8    <NA>    <NA>    <NA>
## Unique (excld. NA)         0         1       1       0       0       0
## Missing values             3         0       0       3       3       3
## Sorted                  <NA>      TRUE    TRUE    <NA>    <NA>    <NA>
##                                                                       
##                       man.ph           app.start             app.end app.method
## Class              character     POSIXct, POSIXt     POSIXct, POSIXt  character
## Minimum                  7.8 2019-05-21 06:00:00 2019-05-21 16:00:00         os
## Maximum                  7.8 2019-05-21 06:00:00 2019-05-21 16:00:00         os
## Mean                    <NA> 2019-05-21 06:00:00 2019-05-21 16:00:00       <NA>
## Unique (excld. NA)         1                   1                   1          1
## Missing values             0                   0                   0          0
## Sorted                  TRUE                TRUE                TRUE       TRUE
##                                                                                
##                    app.rate app.rate.unit    incorp time.incorp man.area
## Class               numeric     character character     numeric  numeric
## Minimum                  30          t/ha      <NA>        <NA>     <NA>
## Maximum                  30          t/ha      <NA>        <NA>     <NA>
## Mean                     30          <NA>      <NA>        <NA>     <NA>
## Unique (excld. NA)        1             1         0           0        0
## Missing values            0             0         3           3        3
## Sorted                 TRUE          TRUE      <NA>        <NA>     <NA>
##                                                                         
##                    dist.inj furrow.z furrow.w      crop  crop.z crop.area
## Class               numeric  numeric  logical character numeric   numeric
## Minimum                  17        4     <NA>     grass      15      <NA>
## Maximum                  17        4     <NA>     grass      15      <NA>
## Mean                     17        4     <NA>      <NA>      15      <NA>
## Unique (excld. NA)        1        1        0         1       1         0
## Missing values            0        0        3         0       0         3
## Sorted                 TRUE     TRUE     <NA>      TRUE    TRUE      <NA>
##                                                                          
##                        lai notes.plot row.in.file.plot  man.trt3 institute
## Class              logical  character          numeric character character
## Minimum               <NA>          .                5      <NA>        AU
## Maximum               <NA>          .                5      <NA>        AU
## Mean                  <NA>       <NA>                5      <NA>      <NA>
## Unique (excld. NA)       0          1                1         0         1
## Missing values           3          0                0         3         0
## Sorted                <NA>       TRUE             TRUE      <NA>      TRUE
##                                                                           
##                    sub.period
## Class                 numeric
## Minimum                     3
## Maximum                     3
## Mean                        3
## Unique (excld. NA)          1
## Missing values              0
## Sorted                   TRUE
##                              
##                                                                         file
## Class                                                              character
## Minimum            ../../data-submitted/03/AU/ALFAM2_JNK_2019_May_5_6_a.xlsx
## Maximum            ../../data-submitted/03/AU/ALFAM2_JNK_2019_May_5_6_a.xlsx
## Mean                                                                    <NA>
## Unique (excld. NA)                                                         1
## Missing values                                                             0
## Sorted                                                                  TRUE
##                                                                             
##                        treat meas.tech meas.tech.det   app.start.orig
## Class              character character     character        character
## Minimum                  AGM       bLS       CRDS_1m 21-05-2019 06:00
## Maximum              CRDS_2m       bLS       CRDS_1m 21-05-2019 06:00
## Mean                    <NA>      <NA>          <NA>             <NA>
## Unique (excld. NA)         3         1             1                1
## Missing values             0         0             0                0
## Sorted                  TRUE      TRUE          TRUE             TRUE
##                                                                      
##                        app.end.orig
## Class                     character
## Minimum            21-05-2019 16:00
## Maximum            21-05-2019 16:00
## Mean                           <NA>
## Unique (excld. NA)                1
## Missing values                    0
## Sorted                         TRUE
##                                    
##                                                                                                                                                                          cpid
## Class                                                                                                                                                               character
## Minimum                D:1.I:AU.Pr:Vejrumbro_2019.F:../../data-submitted/03/AU/ALFAM2_JNK_2019_May_5_6_a.xlsx.E:Vejrumbro_2019_May.F:.P:1.T:AGM.R:1.R2:.T:2019-05-21 06:00:00
## Maximum            D:1.I:AU.Pr:Vejrumbro_2019.F:../../data-submitted/03/AU/ALFAM2_JNK_2019_May_5_6_a.xlsx.E:Vejrumbro_2019_May.F:.P:1.T:CRDS_2m.R:1.R2:.T:2019-05-21 06:00:00
## Mean                                                                                                                                                                     <NA>
## Unique (excld. NA)                                                                                                                                                          3
## Missing values                                                                                                                                                              0
## Sorted                                                                                                                                                                   TRUE
##                                                                                                                                                                              
##                                                               ceid flag.plot
## Class                                                    character character
## Minimum            D:1.I:AU.Pr:Vejrumbro_2019.E:Vejrumbro_2019_May       m i
## Maximum            D:1.I:AU.Pr:Vejrumbro_2019.E:Vejrumbro_2019_May       m i
## Mean                                                          <NA>      <NA>
## Unique (excld. NA)                                               1         1
## Missing values                                                   0         0
## Sorted                                                        TRUE      TRUE
##                                                                             
##                             submitter tan.app     e.1     e.4     e.6    e.12
## Class                       character numeric numeric numeric numeric numeric
## Minimum            Jesper Nørlem Kamp      54    0.18   0.721    1.08    2.04
## Maximum            Jesper Nørlem Kamp      54   0.264    1.05    1.58    2.86
## Mean                             <NA>      54   0.222   0.887    1.33    2.46
## Unique (excld. NA)                  1       1       3       3       3       3
## Missing values                      0       0       0       0       0       0
## Sorted                           TRUE    TRUE   FALSE   FALSE   FALSE   FALSE
##                                                                              
##                       e.24    e.48    e.72    e.96   e.168 e.final   e.tot
## Class              numeric numeric numeric numeric numeric numeric numeric
## Minimum               3.15    4.33    5.22     5.5    <NA>     5.7     5.7
## Maximum               3.48     4.7    5.71    6.05    <NA>    6.33    6.33
## Mean                  3.33    4.47     5.4    5.72    <NA>    5.98    5.98
## Unique (excld. NA)       3       3       3       3       0       3       3
## Missing values           0       0       0       0       3       0       0
## Sorted               FALSE   FALSE   FALSE   FALSE    <NA>   FALSE   FALSE
##                                                                           
##                    e.cum.1 e.cum.4 e.cum.6 e.cum.12 e.cum.24 e.cum.48 e.cum.72
## Class              numeric numeric numeric  numeric  numeric  numeric  numeric
## Minimum               0.18   0.721    1.08     2.04     3.15     4.33     5.22
## Maximum              0.264    1.05    1.58     2.86     3.48      4.7     5.71
## Mean                 0.222   0.887    1.33     2.46     3.33     4.47      5.4
## Unique (excld. NA)       3       3       3        3        3        3        3
## Missing values           0       0       0        0        0        0        0
## Sorted               FALSE   FALSE   FALSE    FALSE    FALSE    FALSE    FALSE
##                                                                               
##                    e.cum.96 e.cum.168 e.cum.final e.cum.tot e.rel.1 e.rel.4
## Class               numeric   numeric     numeric   numeric numeric numeric
## Minimum                 5.5      <NA>         5.7       5.7 0.00334  0.0134
## Maximum                6.05      <NA>        6.33      6.33 0.00488  0.0195
## Mean                   5.72      <NA>        5.98      5.98 0.00411  0.0164
## Unique (excld. NA)        3         0           3         3       3       3
## Missing values            0         3           0         0       0       0
## Sorted                FALSE      <NA>       FALSE     FALSE   FALSE   FALSE
##                                                                            
##                    e.rel.6 e.rel.12 e.rel.24 e.rel.48 e.rel.72 e.rel.96
## Class              numeric  numeric  numeric  numeric  numeric  numeric
## Minimum               0.02   0.0379   0.0583   0.0802   0.0966    0.102
## Maximum             0.0293   0.0529   0.0645   0.0871    0.106    0.112
## Mean                0.0247   0.0456   0.0618   0.0828      0.1    0.106
## Unique (excld. NA)       3        3        3        3        3        3
## Missing values           0        0        0        0        0        0
## Sorted               FALSE    FALSE    FALSE    FALSE    FALSE    FALSE
##                                                                        
##                    e.rel.168 e.rel.final e.rel.tot  rain.1  rain.4  rain.6
## Class                numeric     numeric   numeric numeric numeric numeric
## Minimum                 <NA>       0.106     0.106     0.9     7.8     7.8
## Maximum                 <NA>       0.117     0.117     0.9     7.8     7.8
## Mean                    <NA>       0.111     0.111     0.9     7.8     7.8
## Unique (excld. NA)         0           3         3       1       1       1
## Missing values             3           0         0       0       0       0
## Sorted                  <NA>       FALSE     FALSE    TRUE    TRUE    TRUE
##                                                                           
##                    rain.12 rain.24 rain.48 rain.72 rain.96 rain.168 rain.final
## Class              numeric numeric numeric numeric numeric  numeric    numeric
## Minimum                8.7    14.9      15    19.1    19.1     <NA>       22.6
## Maximum                8.7    14.9      15    19.1    19.1     <NA>       22.6
## Mean                   8.7    14.9      15    19.1    19.1     <NA>       22.6
## Unique (excld. NA)       1       1       1       1       1        0          1
## Missing values           0       0       0       0       0        3          0
## Sorted                TRUE    TRUE    TRUE    TRUE    TRUE     <NA>       TRUE
##                                                                               
##                    rain.tot air.temp.1 air.temp.4 air.temp.6 air.temp.12
## Class               numeric    numeric    numeric    numeric     numeric
## Minimum                22.6       14.6       14.9       15.1        14.4
## Maximum                22.6       14.6       14.9       15.1        14.4
## Mean                   22.6       14.6       14.9       15.1        14.4
## Unique (excld. NA)        1          1          1          1           1
## Missing values            0          0          0          0           0
## Sorted                 TRUE       TRUE       TRUE       TRUE        TRUE
##                                                                         
##                    air.temp.24 air.temp.48 air.temp.72 air.temp.96 air.temp.168
## Class                  numeric     numeric     numeric     numeric      numeric
## Minimum                   12.4        11.4        11.5        11.4         10.9
## Maximum                   12.4        11.4        11.5        11.4         10.9
## Mean                      12.4        11.4        11.5        11.4         10.9
## Unique (excld. NA)           1           1           1           1            1
## Missing values               0           0           0           0            0
## Sorted                    TRUE        TRUE        TRUE        TRUE         TRUE
##                                                                                
##                    air.temp.mn soil.temp.1 soil.temp.4 soil.temp.6 soil.temp.12
## Class                  numeric     numeric     numeric     numeric      numeric
## Minimum                   10.9        14.1        14.1        14.2         14.2
## Maximum                   10.9        14.1        14.1        14.2         14.2
## Mean                      10.9        14.1        14.1        14.2         14.2
## Unique (excld. NA)           1           1           1           1            1
## Missing values               0           0           0           0            0
## Sorted                    TRUE        TRUE        TRUE        TRUE         TRUE
##                                                                                
##                    soil.temp.24 soil.temp.48 soil.temp.72 soil.temp.96
## Class                   numeric      numeric      numeric      numeric
## Minimum                    13.9         13.1           13         12.8
## Maximum                    13.9         13.1           13         12.8
## Mean                       13.9         13.1           13         12.8
## Unique (excld. NA)            1            1            1            1
## Missing values                0            0            0            0
## Sorted                     TRUE         TRUE         TRUE         TRUE
##                                                                       
##                    soil.temp.168 soil.temp.mn soil.temp.surf.1 soil.temp.surf.4
## Class                    numeric      numeric          numeric          numeric
## Minimum                     12.8         12.8             14.9             15.2
## Maximum                     12.8         12.8             14.9             15.2
## Mean                        12.8         12.8             14.9             15.2
## Unique (excld. NA)             1            1                1                1
## Missing values                 0            0                0                0
## Sorted                      TRUE         TRUE             TRUE             TRUE
##                                                                                
##                    soil.temp.surf.6 soil.temp.surf.12 soil.temp.surf.24
## Class                       numeric           numeric           numeric
## Minimum                        15.5              14.6              12.5
## Maximum                        15.5              14.6              12.5
## Mean                           15.5              14.6              12.5
## Unique (excld. NA)                1                 1                 1
## Missing values                    0                 0                 0
## Sorted                         TRUE              TRUE              TRUE
##                                                                        
##                    soil.temp.surf.48 soil.temp.surf.72 soil.temp.surf.96
## Class                        numeric           numeric           numeric
## Minimum                         11.7              11.7              11.6
## Maximum                         11.7              11.7              11.6
## Mean                            11.7              11.7              11.6
## Unique (excld. NA)                 1                 1                 1
## Missing values                     0                 0                 0
## Sorted                          TRUE              TRUE              TRUE
##                                                                         
##                    soil.temp.surf.168 soil.temp.surf.mn  wind.1  wind.4  wind.6
## Class                         numeric           numeric numeric numeric numeric
## Minimum                          11.2              11.2    3.17    3.17    3.17
## Maximum                          11.2              11.2    3.17    3.17    3.17
## Mean                             11.2              11.2    3.17    3.17    3.17
## Unique (excld. NA)                  1                 1       1       1       1
## Missing values                      0                 0       0       0       0
## Sorted                           TRUE              TRUE    TRUE    TRUE    TRUE
##                                                                                
##                    wind.12 wind.24 wind.48 wind.72 wind.96 wind.168 wind.mn
## Class              numeric numeric numeric numeric numeric  numeric numeric
## Minimum               3.25    3.88    3.72    3.91    4.05     3.94    3.94
## Maximum               3.25    3.88    3.72    3.91    4.05     3.94    3.94
## Mean                  3.25    3.88    3.72    3.91    4.05     3.94    3.94
## Unique (excld. NA)       1       1       1       1       1        1       1
## Missing values           0       0       0       0       0        0       0
## Sorted                TRUE    TRUE    TRUE    TRUE    TRUE     TRUE    TRUE
##                                                                            
##                    wind.2m.1 wind.2m.4 wind.2m.6 wind.2m.12 wind.2m.24
## Class                numeric   numeric   numeric    numeric    numeric
## Minimum                 3.17      3.17      3.17       3.25       3.88
## Maximum                 3.17      3.17      3.17       3.25       3.88
## Mean                    3.17      3.17      3.17       3.25       3.88
## Unique (excld. NA)         1         1         1          1          1
## Missing values             0         0         0          0          0
## Sorted                  TRUE      TRUE      TRUE       TRUE       TRUE
##                                                                       
##                    wind.2m.48 wind.2m.72 wind.2m.96 wind.2m.168 wind.2m.mn
## Class                 numeric    numeric    numeric     numeric    numeric
## Minimum                  <NA>       <NA>       <NA>        <NA>       <NA>
## Maximum                  <NA>       <NA>       <NA>        <NA>       <NA>
## Mean                     <NA>       <NA>       <NA>        <NA>       <NA>
## Unique (excld. NA)          0          0          0           0          0
## Missing values              3          3          3           3          3
## Sorted                   <NA>       <NA>       <NA>        <NA>       <NA>
##                                                                           
##                      rad.1   rad.4   rad.6  rad.12  rad.24  rad.48  rad.72
## Class              numeric numeric numeric numeric numeric numeric numeric
## Minimum               71.3     100     107    57.3    44.5     127     148
## Maximum               71.3     100     107    57.3    44.5     127     148
## Mean                  71.3     100     107    57.3    44.5     127     148
## Unique (excld. NA)       1       1       1       1       1       1       1
## Missing values           0       0       0       0       0       0       0
## Sorted                TRUE    TRUE    TRUE    TRUE    TRUE    TRUE    TRUE
##                                                                           
##                     rad.96 rad.168  rad.mn rain.rate.1 rain.rate.4 rain.rate.6
## Class              numeric numeric numeric     numeric     numeric     numeric
## Minimum                160     159     159         0.9        1.95         1.3
## Maximum                160     159     159         0.9        1.95         1.3
## Mean                   160     159     159         0.9        1.95         1.3
## Unique (excld. NA)       1       1       1           1           1           1
## Missing values           0       0       0           0           0           0
## Sorted                TRUE    TRUE    TRUE        TRUE        TRUE        TRUE
##                                                                               
##                    rain.rate.12 rain.rate.24 rain.rate.48 rain.rate.72
## Class                   numeric      numeric      numeric      numeric
## Minimum                   0.725        0.621        0.312        0.265
## Maximum                   0.725        0.621        0.312        0.265
## Mean                      0.725        0.621        0.312        0.265
## Unique (excld. NA)            1            1            1            1
## Missing values                0            0            0            0
## Sorted                     TRUE         TRUE         TRUE         TRUE
##                                                                       
##                    rain.rate.96 rain.rate.168 rain.rate.mn    rh.1    rh.4
## Class                   numeric       numeric      numeric numeric numeric
## Minimum                   0.199         0.156        0.156    91.9    92.7
## Maximum                   0.199         0.156        0.156    91.9    92.7
## Mean                      0.199         0.156        0.156    91.9    92.7
## Unique (excld. NA)            1             1            1       1       1
## Missing values                0             0            0       0       0
## Sorted                     TRUE          TRUE         TRUE    TRUE    TRUE
##                                                                           
##                       rh.6   rh.12   rh.24   rh.48   rh.72   rh.96  rh.168
## Class              numeric numeric numeric numeric numeric numeric numeric
## Minimum               92.8    93.2    93.3    86.7    81.3    80.6    80.7
## Maximum               92.8    93.2    93.3    86.7    81.3    80.6    80.7
## Mean                  92.8    93.2    93.3    86.7    81.3    80.6    80.7
## Unique (excld. NA)       1       1       1       1       1       1       1
## Missing values           0       0       0       0       0       0       0
## Sorted                TRUE    TRUE    TRUE    TRUE    TRUE    TRUE    TRUE
##                                                                           
##                      rh.mn first.row.in.file.int last.row.in.file.int  n.ints
## Class              numeric               numeric              numeric integer
## Minimum               80.7                     5                  280     276
## Maximum               80.7                   559                  834     276
## Mean                  80.7                   282                  557     276
## Unique (excld. NA)       1                     3                    3       1
## Missing values           0                     0                    0       0
## Sorted                TRUE                 FALSE                FALSE    TRUE
##                                                                              
##                        dt1  j.rel1  j.NH31  dt.min  dt.max  ct.min  ct.max
## Class              numeric numeric numeric numeric numeric numeric numeric
## Minimum                  1 0.00334    0.18     0.5       1       1     145
## Maximum                  1 0.00488   0.264     0.5       1       1     145
## Mean                     1 0.00411   0.222     0.5       1       1     145
## Unique (excld. NA)       1       3       3       1       1       1       1
## Missing values           0       0       0       0       0       0       0
## Sorted                TRUE   FALSE   FALSE    TRUE    TRUE    TRUE    TRUE
##                                                                           
##                              t.start.p             t.end.p air.temp.z
## Class                  POSIXct, POSIXt     POSIXct, POSIXt    numeric
## Minimum            2019-05-21 12:00:00 2019-05-27 12:00:00          2
## Maximum            2019-05-21 12:00:00 2019-05-27 12:00:00          2
## Mean               2019-05-21 12:00:00 2019-05-27 12:00:00          2
## Unique (excld. NA)                   1                   1          1
## Missing values                       0                   0          0
## Sorted                            TRUE                TRUE       TRUE
##                                                                      
##                    soil.temp.z  wind.z  wind.loc far.loc
## Class                character numeric character numeric
## Minimum                    0.1       2     Field    <NA>
## Maximum                    0.1       2     Field    <NA>
## Mean                      <NA>       2      <NA>    <NA>
## Unique (excld. NA)           1       1         1       0
## Missing values               0       0         0       3
## Sorted                    TRUE    TRUE      TRUE    <NA>
##                                                         
##                                                                                                                                                                                                                                                                      pub.info
## Class                                                                                                                                                                                                                                                               character
## Minimum            Kamp, J.N.; Häni, C.; Nyord, T.; Feilberg, A.; Sørensen, L.L. Calculation of NH3 Emissions, Evaluation of Backward Lagrangian Stochastic Dispersion Model and Aerodynamic Gradient Method. Atmosphere 2021, 12, 102. https://doi.org/10.3390/atmos12010102
## Maximum            Kamp, J.N.; Häni, C.; Nyord, T.; Feilberg, A.; Sørensen, L.L. Calculation of NH3 Emissions, Evaluation of Backward Lagrangian Stochastic Dispersion Model and Aerodynamic Gradient Method. Atmosphere 2021, 12, 102. https://doi.org/10.3390/atmos12010102
## Mean                                                                                                                                                                                                                                                                     <NA>
## Unique (excld. NA)                                                                                                                                                                                                                                                          1
## Missing values                                                                                                                                                                                                                                                              0
## Sorted                                                                                                                                                                                                                                                                   TRUE
##                                                                                                                                                                                                                                                                              
##                    soil.type2  exper2    rep2    acid meas.tech.orig meas.tech2
## Class                 logical logical logical logical      character  character
## Minimum                  <NA>    <NA>    <NA>   FALSE            bLS  micro met
## Maximum                  <NA>    <NA>    <NA>   FALSE            bLS  micro met
## Mean                     <NA>    <NA>    <NA>  0 TRUE           <NA>       <NA>
## Unique (excld. NA)          0       0       0       1              1          1
## Missing values              3       3       3       0              0          0
## Sorted                   <NA>    <NA>    <NA>    TRUE           TRUE       TRUE
##                                                                                
##                    crop.orig     app.method.orig incorp.orig man.source.orig
## Class              character           character   character       character
## Minimum                Grass Open slot injection        <NA>             Mix
## Maximum                Grass Open slot injection        <NA>             Mix
## Mean                    <NA>                <NA>        <NA>            <NA>
## Unique (excld. NA)         1                   1           0               1
## Missing values             0                   0           3               0
## Sorted                  TRUE                TRUE        <NA>            TRUE
##                                                                             
##                    date.start  flag.int soil.samp.z
## Class                    Date character   character
## Minimum            2019-05-21       m i        <NA>
## Maximum            2019-05-21       m i        <NA>
## Mean                     <NA>      <NA>        <NA>
## Unique (excld. NA)          1         1           0
## Missing values              0         0           3
## Sorted                   TRUE      TRUE        <NA>
## 
```

