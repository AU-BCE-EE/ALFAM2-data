...
p1 <- ggplot(d, aes(ct, e.rel, colour = first.rows)) + 
...
dl <- unique(d[, c('first.rows', 'meas.tech', 'man.source', 'app.method')])
dl <- cbind(x = xl, y = yl, dl)

p1 + geom_text(data = dl, aes(x = x, y = y, label = paste(meas.tech, man.source, app.method, sep = ', '))) +
  geom_line() + geom_point() 
