...
```{r, echo=FALSE}
d$first.row <- factor(paste('Plots', d$row.plot, 'Emissiion', d$first.row))
library(ggplot2)
...
if (!(all(is.na(d$cta)))) {
  ggplot(d, aes(cta, e.rel, colour = first.row)) + geom_line() + geom_point() + theme(legend.position = 'top')
}
...
### Flux
```{r}
ggplot(d, aes(ct, j.NH3, colour = first.row)) + geom_step() + geom_point() + theme(legend.position = 'none') + facet_wrap(~ first.row)
ggplot(d, aes(ct, j.NH3, colour = first.row)) + geom_step() + geom_point() + theme(legend.position = 'top')
if (!(all(is.na(d$cta)))) {
  ggplot(d, aes(cta, j.NH3, colour = first.row)) + geom_step() + geom_point() + theme(legend.position = 'top')
}
```
