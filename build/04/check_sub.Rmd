...
knitr::opts_knit$set(base.dir = normalizePath('../../logs/04/md/'))
knitr::opts_chunk$set(fig.path = paste0(fn, '_files/figure-gfm/'))
```
...
Each of these files contains a summary of the data from a single submitted spreadsheet file.
...
if (all(!is.na(ll))) {
  for (k in 1:nrow(ll)) {
    cat(sprintf('Location %d: lat=%.5f, long=%.5f  [View on map](https://maps.google.com/?q=%.5f,%.5f)\n\n',
                k, ll$lat[k], ll$long[k], ll$lat[k], ll$long[k]))
  }
} else {
  cat('Skipping map: lat/long not provided.\n')
}
```
...
m2 <- merge(m1, emis, all = TRUE, by = c('proj', 'exper', 'pub.id', 'treat'))
...
if (sum(is.na(c(m2$proj, m2$exper, m2$treat, m2$field, m2$plot))) == 0) {
  ...
  md <- m2[is.na(proj) | is.na(exper) | is.na(field) | is.na(plot), .(row.in.file.plot, row.in.file.int, proj, exper, pub.id, treat, field, plot)]
  md <- unique(md)
  print(md)
...
```{r, echo = FALSE}
s <- d[, .(n.int.duplicates = sum(duplicated(interval))), by = cpmid]
...
```{r, echo = FALSE}
ggplot(plots, aes(e.rel.final, e.cum.final)) + geom_point() + facet_grid(meas.tech ~ .) + labs(x = 'Final cum. emission (frac. app. TAN)', y = 'Final cum. emission (kg N/ha)')
```
