# Ideas for submission period 4

1. Check plot and interval counts at the end and compare to previous version (because database can be built without all the data if one file throws an error)
2. Use data.table
3. Convert all spreadsheets to csv files with ssconvert and dump the tidyverse Excel stuff (with pleasure)
4. Always build database with `Rscript main.R` so that an error will stop the whole thing
