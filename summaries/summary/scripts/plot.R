...
dd <- pdat[app.method %in% c('bc', 'bsth', 'ts', 'os', 'cs'), ]
dd[, app.mthd := factor(app.method, levels = c('bc', 'bsth', 'ts', 'os', 'cs'), 
...
ggsave('../plots/ALFAM2_data_summary.pdf', height = 4.3, width = 6.7)
ggsave('../plots/ALFAM2_data_summary.png', height = 4.3, width = 6.7)
...
ggplot(dd, aes(country, e.rel.final, colour = app.mthd, shape = app.mthd)) +
  ...
  coord_cartesian(ylim = c(0, 1.3)) +
  guides(colour = guide_legend(nrow = 2))
ggsave('../plots/ALFAM2_data_summary_AU.pdf', height = 4.3, width = 6.7)
ggsave('../plots/ALFAM2_data_summary_AU.png', height = 4.3, width = 6.7)


dd <- cdat[app.method %in% c('bc', 'bsth', 'ts', 'os', 'cs'), ]
dd[, app.mthd := factor(dd$app.method, levels = c('bc', 'bsth', 'ts', 'os', 'cs'), 
                      labels = c('Broadcast', 'Trailing hose', 'Trailing shoe', 'Open slot injection', 'Closed slot injection'))]

dd[j.NH3.norm < 1E-6, j.NH3.norm := 1E-6]
dd[, x := ct - dt]
dd[, xend := ct]
...
ggsave('../plots/ALFAM2_flux_summary.png', height = 3.3, width = 7)
...
ggplot(dd, aes(x = cta, y = e.rel, colour = app.mthd, group = pmid)) +
  ...
  theme_bw() +
  theme(legend.position = 'none')
ggsave('../plots/ALFAM2_emis_summary.png', height = 3.3, width = 7)
...
dd[, app.mthd := factor(app.method, levels = c('bc', 'bsth', 'ts', 'os', 'cs'), 
                      labels = c('Broadcast', 'Trailing hose', 'Trailing shoe', 'Open slot injection', 'Closed slot injection'))]
dd <- dd[cta >= 0 & pmid %in% pps, ]
d0 <- dd[!duplicated(pmid), .(pmid, app.mthd)]
d0[, cta := 0]
d0[, e.rel := 0]
dd <- rbindf(dd, d0)
ggplot(dd, aes(x = cta, y = e.rel, colour = app.mthd, group = pmid)) +
